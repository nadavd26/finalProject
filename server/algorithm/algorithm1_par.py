...
# The function that handles solving for a single day and skill
def solve_day_skill(
    day, skill, shifts_array, requests_array, skill_mapping, day_mapping
):
    ...
    a = [
        [shifts_array[day][skill][j][0][i] for j in range(num_shifts)]
    ...
    # Return the solved shift workers for the current day and skill
    return [str(int(y[shift].value())) for shift in range(num_shifts)]
...
def solve_shifts(reqs, shifts):
    ...
    for shift in shifts:
        ...
        cost = int(shift[4])
        # Per each shift, append the hours and the cost of it, to the relevant day and skill.
        shifts_array[day_index][skill_index].append((hours, cost))
        output_array[day_index][skill_index].append(
    ...
    # Parallelize the solving for each day and skill
    results = Parallel(n_jobs=-1)(
        delayed(solve_day_skill)(
            day, skill, shifts_array, requests_array, skill_mapping, day_mapping
        )
    ...
    # Flatten and store results back into output_array
    counter = 0
    for day in range(len(day_mapping)):
        for skill in range(len(skill_mapping)):
            num_shifts = len(output_array[day][skill])
            for shift in range(num_shifts):
                output_array[day][skill][shift][4] = results[counter][shift]
            counter += 1

    output = [
        shift
        for day_list in output_array
        for shift_type_list in day_list
